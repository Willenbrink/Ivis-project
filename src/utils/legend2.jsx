...
export function Legend2({svgRef, category, categoryStatistics, range, showRange, selected, colors, markers, zoomCall=false, brushActive=false, setBrushRange, showScaleNumbers=false}){
  ...
  */
  ...
  // 1em = 16 pixels       pixels / 16

  const toEm = (pixels) => { return `${pixels/16}em`}

  const svgIconsHeight = 3 
  const fontSize = 1 // this has to be changed if we change the font or font size
  ...
  const container = {
    x: containerPadding.x,
    y: svgHeight - containerHeight - containerPadding.y, // beror på höjden av content
    width: svgWidth - (containerPadding.x * 2),
    height: containerHeight // beror på höjden av content
  }

  const noDataPadding = 1
  const noDataSettings = {
    x: 0,
    y: 0,
    width: GetWidth(noDataStr) + noDataPadding + boxHeight,
    height: Math.max(boxHeight, fontSize)
  }


  const labelRightSettings = {
    x: 0,
    y: 0,
    width: labelWidths.right,
    height: svgIconsHeight + (fontSize * 2) // every label has one icon + two rows of text
  }

  const labelleftSettings = {
    x: 0,
    y: 0,
    width: labelWidths.left,
    height: svgIconsHeight + (fontSize * 2) // every label has one icon + two rows of text
  }
...
const noDataSvg = (
  <svg width="100%" height="100%" viewBox={`0 0 ${noDataSettings.width} ${noDataSettings.height}`} fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
    <text fontSize={noDataText.fontSize} x={0} y={fontSize} width={toEm(noDataText.width)} height={toEm(noDataText.height)} fill={noDataText.color}>{noDataStr}</text>
    <rect x={noDataText.width + noDataPadding} width={'1em'} height={'1em'} fill={colorScheme.noData} stroke="#333" strokeWidth="0.3" rx={roundedCorners}></rect>
  </svg>
)
/*
const leftLabelSvg = (
...
const ColorBoxSvg = (showScaleNumbers) => {
  <svg>
  {showScaleNumbers && (
        <>
 ...
 const styleTransition = {transition: "0.3s"}
  ...
  return (
    <svg height='100%' width='100%' fontFamily={fontFamily} className=''>
        <svg x={container.x} y={container.y} width={container.width} height={container.height} filter="drop-shadow(3px 5px 2px rgb(0 0 0 / 0.1))">
          <rect width='100%' height='100%' fill='rgb(256,256,256)' rx="15" stroke='rgb(206,212,218)' strokeWidth={1} fillOpacity='100%'></rect>
          {noDataSvg}
        </svg>
