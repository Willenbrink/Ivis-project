...
export default function CategorySelectorInfo({setCategory, category, isActiveTab}){
  return(
    <InputGroup className="px-5 pt-2 position-absolute" style={{width: "90%"}}>
      ...
      <InfoPopover title={categories[category.id].name_short || categories[category.id].name} info={categories[category.id].info} isActiveTab={isActiveTab} className="info-box"/>
    </InputGroup>
  )
}